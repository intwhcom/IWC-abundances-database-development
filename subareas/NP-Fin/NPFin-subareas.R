...
areasENP <- st_intersection(st_make_valid(cliparea), areasNP) %>% mutate(subareas = "E.N. Pacific", accuracy = "accurate", large_area = "NP", species = "Fin", survey = "")
...
Hawaii <- eezs %>% filter(UNION == "Hawaii") %>% rename(subareas = UNION) %>%
  select(subareas,geometry) %>% mutate(species = "Fin", accuracy = "approximate", large_area="NP", subareas = "Hawaiian EEZ")

#### final output ####
output <- dplyr::bind_rows(list(Hawaii, areasENP)) 
...
#save 
st_write(output, "C:/Users/IsidoraKatara/OneDrive - International Whaling Commission/Abundance-Tabs&Refs/Extras/mapping subareas/NP-Fin/NPFinSubareas.shp",append=FALSE)
